...
class Explossions {
  constructor(x, y, size) {
    ...
    this.sound.src = "./assets/blast.mp3";
...
function drawScore() {
  ctx.fillStyle = "black";
  ctx.fillText(`Score: ${score}`, 60, 95);
  ctx.fillStyle = "white";
  ctx.fillText(`Score: ${score}`, 65, 100);
}
...
window.addEventListener("click", (e) => {
  ...
  crows.forEach((object) => {
    if (
      ...
      score++;
      explosions.push(new Explossions(object.x, object.y, object.width));
    }
  });
});
function animate(timestamp) {
  ...
  if (timeToNextFrame > crowInterval) {
    crows.push(new Crow());
    timeToNextFrame = 0;
    crows.sort((a, b) => a.width - b.width);
  }
  drawScore();
  [...particles, ...crows, ...explosions].forEach((object) => {
    object.update(deltaTime);
  });
  crows = crows.filter((object) => !object.markedForDeletion);
